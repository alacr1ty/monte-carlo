...
main( int argc, char *argv[ ] )
{
    ...
    float currentProb;              // must be declared outside the NUMTRIES loop
   	double execution_min = numeric_limits<double>::infinity();
...
// Otherwise, this beam hit the infinite plate. (Case D) Increment the number of hits and continue on to the next trial in the for-loop.
		...
		double execution = 1000000. * (time1 - time0);

		if( megaTrialsPerSecond > maxPerformance )
			maxPerformance = megaTrialsPerSecond;

		if (execution < execution_min)
	...
	fprintf(stderr, "Probability of Hitting the Plate: %.6f\n", currentProb);
	fprintf(stderr, "Mega Trials per Second: %8.2lf\n", maxPerformance);
	fprintf(stderr, "Execution Speed: %8.2lf\n\n", execution_min);
